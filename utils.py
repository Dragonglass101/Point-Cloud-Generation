...
def make_data_fixed(cfg):
    ...
    dl_tr = DataLoader(
        ...
        drop_last=True, collate_fn=ds_tr.collate_fn_fixed, num_workers=0)
    ...
    dl_test = DataLoader(
        ...
        drop_last=True, collate_fn=ds_test.collate_fn_fixed, num_workers=0)
...
def make_data_novel(cfg):
    ...
    dl_tr = DataLoader(
        ds_tr, batch_size=cfg.chunkSize, shuffle=True,
        drop_last=True, collate_fn=ds_tr.collate_fn, num_workers=0)

    ds_test = data.PointCloud2dDataset(
        cfg, loadNovel=True, loadFixedOut=False, loadTest=True)
    dl_test = DataLoader(
        ds_test, batch_size=cfg.chunkSize, shuffle=False,
        drop_last=True, collate_fn=ds_test.collate_fn, num_workers=0)
...
def build_structure_generator(cfg):
    ...
    if cfg.loadPath is not None:
        print("Best model path: " + str(cfg.loadPath))
