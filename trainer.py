...
class TrainerStage1:
    ...
    def train(self, model, optimizer, scheduler):
        ...
        print("Start Epoch: " + str(self.cfg.startEpoch) + "        End Epoch: " + str(self.cfg.endEpoch))
...
class TrainerStage2:
    ...
    def train(self, model, optimizer, scheduler):
        print("======= TRAINING START =======")
        # self.cfg.endEpoch
        for self.epoch in range(self.cfg.startEpoch, 5):
            print(f"Epoch {self.epoch}:")
...
class Validator:
    ...
    def eval(self, model):
        ...
        for i in range(len(self.dataset)):
            ...
            # XYZid_cpu = XYZid.detach().cpu().numpy()
            # ML_cpu = ML.detach().cpu().numpy()
            # ML_reshape = ML_cpu[:, :, np.newaxis]
            # combined_array = np.concatenate((XYZid_cpu, ML_reshape), axis=2)
            # np.save(f"{save_dir}/{self.CADs[i]}.npy", combined_array)
    ...
    def eval_dist(self):
        ...
        with torch.set_grad_enabled(False):
            for m, cad in enumerate(self.CADs):
                ...
                print(f"{self.result_path}/{cad}.mat", len(Vpred24), self.cfg.inputViewN)
                print(Vpred24[0].shape)
